    ...
    public static class OrcCompressedStream
    {
        ...
        public static IOStream GetDecompressingStream(IOStream inputStream, CompressionKind compressionKind)
        {
            ...
            return new ConcatenatingStream(() =>
            {
                int blockLength;
                bool isCompressed;
                var headerAvailable = ReadBlockHeader(inputStream, out blockLength, out isCompressed);
                if (!headerAvailable)
                    return null;
        ...
        private static void WriteBlockHeader(IOStream outputStream, int blockLength, bool isCompressed)
        {
            if (blockLength > 0x7FFFFF)
                throw new OverflowException(
                    $"Compressed block size cannot be larger than 8*3-1 bits (8MB). Attempted to store {blockLength} bytes.");
            var value = blockLength << 1;
            if (!isCompressed)
                value |= 1;
