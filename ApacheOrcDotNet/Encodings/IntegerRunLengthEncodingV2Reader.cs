...
namespace ApacheOrcDotNet.Encodings
{
    public class IntegerRunLengthEncodingV2Reader
    {
        ...
        private IEnumerable<long> ReadShortRepeatValues(int firstByte)
        {
        ...
        private IEnumerable<long> ReadDirectValues(int firstByte)
        {
            var encodedWidth = (firstByte >> 1) & 0x1f;
            var width = encodedWidth.DecodeDirectWidth();
            var length = (firstByte & 0x1) << 8;
        ...
        private IEnumerable<long> ReadPatchedBaseValues(int firstByte)
        {
            var encodedWidth = (firstByte >> 1) & 0x1f;
            var width = encodedWidth.DecodeDirectWidth();
            var length = (firstByte & 0x1) << 8;
            ...
            var baseValue = _inputStream.ReadLongBE(baseValueWidth);
            var msbMask = 1L << (baseValueWidth * 8 - 1);
            ...
            if (patchGapWidth + patchWidth > 64)
                throw new InvalidDataException(
                    $"{nameof(patchGapWidth)} ({patchGapWidth}) + {nameof(patchWidth)} ({patchWidth}) > 64");
            ...
            for (var i = 0; i < length; i++)
                if (i == gap)
                {
                    ...
                    if (patchIndex < patchListLength)
                        GetNextPatch(patchListValues, ref patchIndex, ref gap, out patch, patchWidth,
                            (1L << patchWidth) - 1);
                }
                else
                {
        ...
        private void GetNextPatch(long[] patchListValues, ref int patchIndex, ref long gap, out long patch,
            int patchWidth, long patchMask)
        {
        ...
        private IEnumerable<long> ReadDeltaValues(int firstByte)
        {
            var encodedWidth = (firstByte >> 1) & 0x1f;
            var width = 0;
            ...
            long currentValue;
            currentValue = _isSigned ? _inputStream.ReadVarIntSigned() : _inputStream.ReadVarIntUnsigned();
