...
namespace ApacheOrcDotNet.Encodings
{
    public class IntegerRunLengthEncodingV2Writer
    {
        ...
        private int EncodeValues(IList<long> values, bool areSigned, bool aligned)
        {
            //Eventually:
            ...
            if (result == DeltaEncodingResult.Success)
                return length;
            if (result == DeltaEncodingResult.Overflow)
            {
                var directZigZaggedValues = areSigned ? values.ZigzagEncode() : values;
                DirectEncode(directZigZaggedValues, values.Count, aligned, fixedBitWidth);
                return values.Count;
        ...
        private void FindRepeatedValues(IEnumerable<long> values, out long repeatingValue, out int length)
        {
            length = 0;
            repeatingValue = 0;
        ...
        private bool SequenceIsTooShort(IList<long> values)
        {
            if (values.Count <= 3)
                return true;
            return false;
        ...
        private void DeltaEncode(long initialValue, bool areSigned, int repeatCount)
        {
            DeltaEncode(initialValue, areSigned, 0, repeatCount);
        }

        private void DeltaEncode(long initialValue, bool areSigned, long constantOffset, int repeatCount)
        {
            DeltaEncode(initialValue, areSigned, repeatCount, new[] {constantOffset}, 0);
        }

        private void DeltaEncode(long initialValue, bool areSigned, int numValues, long[] deltas, int deltaBitWidth)
        {
            var encodedBitWidth = 0;
            if (deltaBitWidth > 0)
        ...
        private enum DeltaEncodingResult
        {
            Success,
            Overflow,
            NonMonotonic
