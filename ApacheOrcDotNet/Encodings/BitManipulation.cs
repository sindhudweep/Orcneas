...
namespace ApacheOrcDotNet.Encodings
{
    public static class BitManipulation
    {
        ...
        public static float ReadFloatBE(this byte[] buffer, int startIndex)
        {
            ...
            for (var i = reverseBuf.Length - 1; i >= 0; i--)
        ...
        public static void WriteLongBE(this Stream stream, int numBytes, long value)
        {
            for (var i = numBytes - 1; i >= 0; i--)
            {
                var nextByte = (byte) ((ulong) value >> (i * 8));
        ...
        {
            return values.Select(ZigzagEncode);
        }
        ...
        public static int GetBitsRequiredForPercentile(Tuple<int, int[]> histogramTuple, double percentile)
        {
            ...
            var numValuesToDrop = (int) (totalNumValues * (1.0 - percentile));
