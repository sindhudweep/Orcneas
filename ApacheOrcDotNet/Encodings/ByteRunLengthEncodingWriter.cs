...
namespace ApacheOrcDotNet.Encodings
{
    public class ByteRunLengthEncodingWriter
    {
        ...
        private void EncodeLiterals(IList<byte> values)
        {
        ...
        private int FindNonRepeatingValues(IList<byte> values)
        {
            if (values.Count < 3)
                return values.Count;
        ...
        private int FindRepeatedValues(IList<byte> values, out byte repeatingValue)
        {
            var result = 0;
