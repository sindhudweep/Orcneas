...
namespace ApacheOrcDotNet.Infrastructure
{
    public class StreamSegment : Stream
    {
        ...
        public override long Length =>
            _lengthToExpose; //What if the underlying Stream has less bytes available than this?
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            var bytesToRead =
                (int) Math.Min(count,
                    remainingBytes); //Safe to cast to an int here because count can never exceed Int32.MaxValue
