...
namespace ApacheOrcDotNet.ColumnTypes
{
    public class TimestampReader : ColumnReader
    {
        ...
        public TimestampReader(StripeStreamReaderCollection stripeStreams, uint columnId) : base(stripeStreams,
            columnId)
        {
        }
        ...
        public IEnumerable<DateTime?> Read()
        {
            ...
            var secondary = ReadNumericStream(StreamKind.Secondary, false);
            ...
            if (present == null)
                while (dataEnumerator.MoveNext() && secondaryEnumerator.MoveNext())
                {
                    ...
                    var totalTicks = seconds * TimeSpan.TicksPerSecond +
                                     (seconds >= 0 ? nanosecondTicks : -nanosecondTicks);
